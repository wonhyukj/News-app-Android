...
function validGuardianHome(temp) {
    console.log(temp)
    if(temp.webPublicationDate == null || temp.webPublicationDate.length == 0) {
        return false
    }
    ...
    if(temp.sectionName == null || temp.sectionName.length == 0) {
        return false
    } 
    return true
}
...
function cleaningGuardian(temp) {
    ...
    for(var i = 0; i < temp.length; i++) {
        ...
        if(validGuardian(temp[i])) {
            ...
            if(temp[i].blocks.body[0].bodyHTML) {
                temp[i].blocks.body[0].bodyHTML = temp[i].blocks.body[0].bodyHTML
            }
...
function cleaningGuardianHome(temp) {
    console.log(temp)
    for(var i = 0; i < temp.length; i++) {
        flag = 0
        if(validGuardianHome(temp[i])) {
            try {
                if(temp[i].fields.thumbnail)
                    temp[i]['bigUrl'] = temp[i].fields.thumbnail
                else
                    temp[i]['bigUrl'] = Guardians_hq.png
...
app.get("/Guardian_Home", (req, res) => {
    request("https://content.guardianapis.com/search?order-by=newest&show-fields=starRating,headline,thumbnail,short-url&api-key=c6aa0af3-1235-4cfe-84dc-310b41f9e231",
            function(error, response, body) {
                if(!error && response.statusCode == 200) {
                    var parsedBody = JSON.parse(body)
                    var temp = parsedBody['response']['results']
                    cleaningGuardianHome(temp)
                    temp = temp.filter(function(a){return typeof a !== 'null';})
                    res.send({temp:temp.slice(0,10)})
                }
            }
...
app.get("/Guardian_Science", (req, res) => {
    request("https://content.guardianapis.com/science?api-key=c6aa0af3-1235-4cfe-84dc-310b41f9e231&show-blocks=all&page-size=100",
            function(error, response, body) {
                if(!error && response.statusCode == 200) {
                    var parsedBody = JSON.parse(body)
                    var temp = parsedBody['response']['results']
...
app.get("/article", (req, res) => {
        ...
        request(queryLink,
            function(error, response, body) {
                if(!error && response.statusCode == 200) {
                    ...
                    var bodyHTML = ""
                    var body = temp.blocks.body;
                    // console.log(body);
                    // console.log(body.length);
                    for(var i = 0; i < body.length; i++) {
                        bodyHTML += body[i].bodyHtml;
                    }
                    temp['long_desc'] = bodyHTML;
    ...
    app.get("/Trend", (req, res) => {
        const param = req.query;
        var queryLink;
        googleTrends.interestOverTime({keyword: param.keyword, startTime: new Date('2019-06-01'), geo: "US"}) 
            .then(function(results){
                console.log('These results are awesome', results);
                res.send({temp:JSON.parse(results)});
            })
            .catch(function(err){
                res.send({temp:[]});
            });
